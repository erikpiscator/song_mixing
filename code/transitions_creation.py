...
def select_cue_points(previous_mix):
    ...
    cue_point_idx = possible_idx[possible_idx > previous_mix["cue_points"].size - max_transition_length*previous_mix["frame_rate"]][0]    
...
def time_wrap(previous_mix, next_song, previous_mix_cue_point):
    ...
    ending_audio_stretched = rb.stretch(np.array(ending_audio, dtype='int32'),rate=previous_mix['frame_rate'],ratio=ending_stretching_ratio,crispness=5,formants=False,precise=True)
    beginning_audio_stretched = rb.stretch(np.array(beginning_audio, dtype='int32'),rate=next_song['frame_rate'],ratio=beginning_stretching_ratio,crispness=5,formants=False,precise=True)
    ...
    new_next = next_song.copy()
    new_next['audio_array'] = np.concatenate((beginning_audio_stretched, new_next['audio_array'][beginning_length_samples:]))
    new_next['beat_times'] = np.concatenate((beginning_beats_stretched, new_next['beat_times'][beginning_length_samples:]))
    new_next['cue_points'] = np.concatenate((np.zeros(beginning_audio_stretched.size, dtype=next_song['cue_points'].dtype), next_song['cue_points'][beginning_length_samples:]))
